...
export default function ArticleEditForm() {
  ...
  const { data: art, isLoading } = useArticle(id)
  const { mutate, isLoading: rqLoading } = useUpdateArticle()
  ...
  useEffect(() => {
    if (art) {
      setForm({ title: art.title, description: art.description, photo: null })
      setPreview(art.photoUrl)
    }
  }, [art])
  ...
  const handleSubmit = e => {
    ...
    mutate(
      { id, formData: fd },
      {
        onSuccess: ()=>navigate(`/articles/${id}`),
      ...
      <div className="flex-1 p-8">
        <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg relative">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              {isBusy?'Updating…':'Update Article'}
            </button>
          </form>
        </div>
      </div>
